...
func Open(ctx context.Context, js jetstream.JetStream, name string) (Directory, error) {
	store, err := js.CreateKeyValue(ctx, jetstream.KeyValueConfig{
		Bucket: name,
	})
	if err != nil {
		return Directory{}, translate(err)
	}
	return Directory{
		store: store,
	}, nil
...
func (d *Directory) Get(ctx context.Context, key string) (DirectoryEntry, error) {
	...
	if err != nil {
		return DirectoryEntry{}, translate(err)
	}
