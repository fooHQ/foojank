	...
	for {
		for {
			...
			case msg := <-inputCh:
				...
				resp := callWithTimeout(ctx, 5*time.Second, fn)
...
func callWithTimeout(ctx context.Context, timeout time.Duration, fn func(context.Context) message.Msg) message.Msg {
