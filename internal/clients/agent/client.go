	...
	return jwt.Permissions{
		Pub: jwt.Permission{
			Allow: []string{
				...
				fmt.Sprintf("$JS.ACK.%s.>", agentID),
				fmt.Sprintf("$JS.API.STREAM.INFO.%s", agentID),
				fmt.Sprintf("$JS.API.STREAM.INFO.OBJ_%s", agentID),
				fmt.Sprintf("$JS.API.STREAM.PURGE.OBJ_%s", agentID),
				fmt.Sprintf("$JS.API.CONSUMER.INFO.%s.*", agentID),
				fmt.Sprintf("$JS.API.CONSUMER.MSG.NEXT.%s.*", agentID),
