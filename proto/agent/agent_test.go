...
func TestMarshalUnmarshal(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Envelope with empty fields",
			input: agent.Envelope{
				JWT:       "",
				WorkerID:  "",
				Payload:   []byte{},
...
func TestSignVerify(t *testing.T) {
	...
	marshaled, err := agent.Marshal(env)
	require.NoError(t, err)

	unmarshaled, err := agent.Unmarshal(marshaled)
	require.NoError(t, err)

	got, ok := unmarshaled.(agent.Envelope)
	require.True(t, ok)
...
func TestVerifyTamper(t *testing.T) {
	...
	tests := []struct {
		name   string
		mutate func(e *agent.Envelope)
	}{
		{
			name:   "tampered worker",
			mutate: func(e *agent.Envelope) { e.WorkerID = "worker2" },
		},
		{
			name:   "tampered payload",
			mutate: func(e *agent.Envelope) { e.Payload = []byte("other") },
		},
		{
			name:   "tampered signature",
			mutate: func(e *agent.Envelope) { e.Signature[0] ^= 0xFF },
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := env.Verify(pub)
			require.ErrorIs(t, err, agent.ErrInvalidSignature)
		})
	}
}
...
func TestVerifyWrongKey(t *testing.T) {
	...
	env := agent.Envelope{WorkerID: "worker1", Payload: []byte("payload")}
	err = env.Sign(kp)
	require.NoError(t, err)
...
func TestVerifyInvalidPublicKey(t *testing.T) {
	env := agent.Envelope{WorkerID: "worker1", Signature: []byte("sig")}

	err := env.Verify("not-a-public-key")
	require.Error(t, err)
	require.NotErrorIs(t, err, agent.ErrInvalidSignature)
}
