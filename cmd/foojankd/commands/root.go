...
func action(ctx context.Context, c *cli.Command) error {
	...
	_, err = srv.CreateStream(ctx, jetstream.StreamConfig{
		Name: streamName,
		Subjects: []string{
	...
	gatewayDir, err := directory.OpenGatewayDirectory(ctx, srv)
	if err != nil {
		logger.ErrorContext(ctx, "Cannot open gateway directory: %v", err)
		return err
	}
